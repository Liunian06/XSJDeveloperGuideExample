...
- [ ] 实现联系人 App 页面
- [ ] 实现记忆 App 页面（完整功能：列表、搜索、标签筛选、重要度、新增/编辑/删除、详情查看）
- [ ] 实现日记 App 页面（完整功能：列表、搜索、情绪筛选、月份分组、新增/编辑/删除、详情查看）
- [ ] 实现论坛 App 页面（完整功能：列表、搜索、标签筛选、点赞、新增/编辑/删除、详情查看）
- [ ] 实现世界书 App 页面
- [ ] 实现预设 App 页面
...
### 3.1 启动、解锁与应用生命周期
- [x] 实现应用启动流程：进入应用先展示锁屏，再进入主屏
- [x] 设计冷启动/热启动状态机：`cold_start`、`locked`、`unlocked`、`background`、`restored`
- [x] 首次进入判定逻辑：数据库无密码时进入引导设置，否则进入输入密码流程
- [x] 实现 6 位数字密码设置流程：输入两次一致校验、弱密码提示（如 `123456`/`000000`）
- [x] 实现密码输入与解锁流程：逐位输入、退格删除、提交校验、失败反馈
- [x] 增加连续失败保护：失败次数统计、短暂锁定、倒计时后可重试
- [ ] 支持锁屏快捷操作：长时间无操作自动锁屏（返回主屏不自动上锁）
- [ ] 支持会话恢复：解锁后恢复上次打开应用与滚动位置（可配置）
- [ ] 明确生命周期事件：页面可见性切换、刷新恢复、离线恢复、异常崩溃恢复

### 3.2 全局路由、导航与上下文传递
- [x] 实现主屏应用启动逻辑（点击不同应用进入对应页面）
- [x] 设计统一路由规范：页面标识、参数命名、返回栈规则、深链接约定
- [x] 实现页面间跳转与上下文传递（URL 参数或路由状态）
- [x] 实现统一导航容器：顶部标题、返回按钮、二级页面层级展示
...
### 3.8 记忆 App（独立需求拆解）
- [ ] 记忆条目新增/编辑/删除：标题、正文、标签、重要度、时间戳
- [ ] 分类体系：按主题标签、人物标签、场景标签多维过滤
- [ ] 重要度机制：星级或优先级，支持仅看高优先记忆
- [ ] 关联能力：记忆与联系人、会话、日记、世界书条目互链
- [ ] 复习机制（可选）：按遗忘曲线生成复习列表与提醒
- [ ] 搜索能力：全文检索 + 标签筛选 + 时间范围筛选

### 3.9 日记 App（独立需求拆解）
- [ ] 按日历写作：日视图快速新建、补写历史日期、当天草稿自动恢复
- [ ] 日记列表：按月分组、摘要预览、字数统计、最近编辑时间
- [ ] 情绪标签：多选标签、情绪趋势统计（周/月）
- [ ] 全文检索：关键词检索、日期范围过滤、标签过滤
- [ ] 写作体验：自动保存、撤销恢复、超长文本分段渲染
- [ ] 隐私保护：可选本地二级加锁（进入日记页再次验证）

### 3.10 论坛 App（独立需求拆解）
- [ ] 帖子列表：按最新/最热排序，支持标签筛选与分页加载
- [ ] 发帖与编辑：标题、正文、标签、草稿暂存、发布校验
- [ ] 评论回复：楼中楼结构、@回复、时间排序
- [ ] 点赞收藏：防重复点赞、取消点赞、收藏列表管理
- [ ] 内容治理：敏感词拦截、超长内容限制、空内容防提交
- [ ] 个人视图：我的帖子、我的评论、我的收藏
...
## 四、本地数据库（IndexedDB）
- [x] 封装 IndexedDB 存储模块（建库、事务、CRUD、容错）
- [x] 设计对象仓库与索引（`settings`、`contacts`、`conversations`、`messages`、`groups`、`moments`、`memories`、`journals`、`forums`、`worldbook`、`presets`、`stickers`）
- [x] 在 `settings` 中存储解锁密码（作为本地功能特性使用，不做哈希）
- [x] 在 `settings` 中实现数据库导入导出（JSON 备份/恢复）
- [ ] 保存并持久化各 App 数据（刷新后可恢复）
- [ ] 首次启动初始化默认联系人与示例内容（聊天、群聊、朋友圈、论坛、世界书等）
- [x] 增加配额阈值策略：当数据达到配额的 70%/80%/90% 时，自动申请更稳定的存储能力（`navigator.storage.persist()`）；若申请成功则继续扩容使用，若申请失败则提示用户当前用量已接近上限并引导清理旧数据
- [x] 增加存储持久化申请（`navigator.storage.persist()`）与失败兜底策略
- [ ] 增加 IndexedDB 索引优化：为高频查询字段建立复合索引并定期评估读写性能
- [ ] 增加迁移脚本（可选）：将旧 `localStorage` 聊天数据迁移到 `IndexedDB`

## 五、体验优化与质量保障
- [x] 增加基础交互反馈（解锁失败震动/提示、发送动画、按钮可用态、空状态）
